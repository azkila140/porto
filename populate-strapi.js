// Automated Strapi Content Population Script
// Run this after creating the content types in Strapi admin
...
async function createEntry(contentType, data, locale = 'en') {
...
async function createLocalization(contentType, entryId, locale, data) {
    const response = await fetch(`${STRAPI_URL}/api/${contentType}/${entryId}/localizations`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${API_TOKEN}`
...
async function publishEntry(contentType, entryId) {
    const response = await fetch(`${STRAPI_URL}/api/${contentType}/${entryId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${API_TOKEN}`
...
async function populateServices() {
    ...
    for (const service of servicesData) {
        try {
            ...
            const { order, icon, gradient, imageUrl, localizations } = service;
            const enData = {
                ...localizations.en,
                order,
                icon,
            ...
            const created = await createEntry('services', enData, 'en');
            const entryId = created.data.id;
            console.log(`✅ Created service: ${localizations.en.title}`);

            // Create Arabic localization
            await createLocalization('services', entryId, 'ar', localizations.ar);
            console.log(`  ✅ Added Arabic translation`);

            // Create French localization
            await createLocalization('services', entryId, 'fr', localizations.fr);
            console.log(`  ✅ Added French translation`);

            // Publish
            await publishEntry('services', entryId);
...
async function populatePortfolio() {
    console.log('\n🔵 Populating Portfolio Items...');

    for (const item of portfolioData) {
        try {
            const { order, featured, gradient, imageUrl, localizations } = item;
            const enData = {
                ...localizations.en,
                order,
                featured,
            ...
            const created = await createEntry('portfolio-items', enData, 'en');
            const entryId = created.data.id;
            console.log(`✅ Created portfolio: ${localizations.en.title}`);

            await createLocalization('portfolio-items', entryId, 'ar', localizations.ar);
            console.log(`  ✅ Added Arabic translation`);

            await createLocalization('portfolio-items', entryId, 'fr', localizations.fr);
            console.log(`  ✅ Added French translation`);

            await publishEntry('portfolio-items', entryId);
            console.log(`  ✅ Published`);
...
async function populateHeroSlides() {
    console.log('\n🔵 Populating Hero Slides...');

    for (const slide of heroData) {
        try {
            const { order, active, icon, gradient, link, imageUrl, localizations } = slide;
            const enData = {
                ...localizations.en,
                order,
                active,
            ...
            const created = await createEntry('hero-slides', enData, 'en');
            const entryId = created.data.id;
            console.log(`✅ Created hero slide: ${localizations.en.title}`);

            await createLocalization('hero-slides', entryId, 'ar', localizations.ar);
            console.log(`  ✅ Added Arabic translation`);

            await createLocalization('hero-slides', entryId, 'fr', localizations.fr);
            console.log(`  ✅ Added French translation`);

            await publishEntry('hero-slides', entryId);
            console.log(`  ✅ Published`);
...
async function main() {
    ...
    try {
        ...
        console.log('\n📝 Next steps:');
        console.log('1. Check Strapi admin to verify all content');
        console.log('2. Test the APIs:');
        console.log('   - http://localhost:1337/api/services?locale=ar');
        console.log('   - http://localhost:1337/api/portfolio-items?locale=fr');
        console.log('   - http://localhost:1337/api/hero-slides?locale=en');
