...
export default async function AdminLayout({
    ...
    const navItems = {
        ar: [
            { href: `/${lang}/admin`, icon: LayoutDashboard, label: 'نظرة عامة' },
            { href: `/${lang}/admin/hero`, icon: Layers, label: 'الواجهة الرئيسية' },
            { href: `/${lang}/admin/services`, icon: Palette, label: 'الخدمات' },
            { href: `/${lang}/admin/leads`, icon: Users, label: 'الطلبات' },
            { href: `/${lang}/admin/settings`, icon: Settings, label: 'الإعدادات' },
        ],
        fr: [
            { href: `/${lang}/admin`, icon: LayoutDashboard, label: 'Aperçu' },
            { href: `/${lang}/admin/hero`, icon: Layers, label: 'Hero Section' },
            { href: `/${lang}/admin/services`, icon: Palette, label: 'Services' },
            { href: `/${lang}/admin/leads`, icon: Users, label: 'Leads' },
            { href: `/${lang}/admin/settings`, icon: Settings, label: 'Paramètres' },
        ],
        en: [
            { href: `/${lang}/admin`, icon: LayoutDashboard, label: 'Overview' },
            { href: `/${lang}/admin/hero`, icon: Layers, label: 'Hero Section' },
            { href: `/${lang}/admin/services`, icon: Palette, label: 'Services' },
            { href: `/${lang}/admin/leads`, icon: Users, label: 'Leads' },
            { href: `/${lang}/admin/settings`, icon: Settings, label: 'Settings' },
        ],
    }
            ...
            <header className="bg-slate-900/50 backdrop-blur-md border-b border-white/10 sticky top-0 z-50">
                <div className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href={`/${lang}`} className="text-xl font-bold bg-gradient-to-r from-[#C5A059] to-[#8B7355] bg-clip-text text-transparent">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <form action="/auth/signout" method="post">
                            ...
                            >
                                ...
                                <span className="text-sm font-semibold">{lang === 'ar' ? 'خروج' : 'Logout'}</span>
                            </button>
                        </form>
                    </div>
                </div>
